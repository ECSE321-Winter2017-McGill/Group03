...
//		for (Applicant anApplicant: ms.getApplicants()) {
//			for (Application aApplication: anApplicant.getApplications()){
//				if (aApplication.getJobPosting().getCourse().getCourseName().equals(course.getCourseName())){
		...
		acceptAppButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (allApplication.getItemCount()==0){
					...
					int reply = JOptionPane.showConfirmDialog(AllocationPage.this, "Hire for all tutorial/lab sessions?", null, JOptionPane.YES_NO_OPTION);
			        if (reply == JOptionPane.YES_OPTION) {
			        	valueSet = course.getLabHour()*course.getNumLab()+course.getTutorialHour()*course.getNumTutorial();
...
//		
	...
	private void checkGradButtonActionPerformed(java.awt.event.ActionEvent evt){
		...
		JOptionPane.showMessageDialog(AllocationPage.this,"There are " + numGradStudent + " grad applicants:\n"
				+ gradNames);
