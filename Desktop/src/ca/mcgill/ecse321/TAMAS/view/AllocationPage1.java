...
public class AllocationPage1 extends JFrame {
	...
	public AllocationPage1(ManagementSystem ms, Course course, Object user) {
		this.ms = ms;
		this.course = course;
		this.user = user;
			...
			public void actionPerformed(ActionEvent e) {
				if (allApplication.getItemCount() == 0) {
					...
					if (tc.applicationRejected(selectedApp)) {
						JOptionPane.showMessageDialog(AllocationPage1.this, "Application already rejected.");
					} else {
	...
	private void checkGradButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		JOptionPane.showMessageDialog(AllocationPage1.this,
	...
	private void modifyButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		for (int i = 0; i < data.length; i++) {
			try {
				if (table.getModel().getValueAt(i, 2).equals("TA")) {
//					System.out.println(nameApplicantMap.get(table.getModel().getValueAt(i, 0)).getName());
